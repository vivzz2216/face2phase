...
from fastapi import FastAPI, File, UploadFile, HTTPException, BackgroundTasks, Depends, Header, Request, Body
...
from models.audio_analyzer import audio_analyzer
from models.enhanced_audio_analyzer import enhanced_audio_analyzer
from models.pdf_report_generator import pdf_word_generator
from models.facial_analyzer import facial_analyzer
...
from models.strict_audio_evaluator import StrictAudioEvaluator
...
from models.detailed_feedback_generator import detailed_feedback_generator
from models.chatbot_service import chatbot_service
from models.pro_pdf_exporter import pro_pdf_exporter
from models.export_job_manager import export_job_manager
from models.transcript_processor import transcript_processor
from models.word_analyzer import word_analyzer
...
from config import UPLOAD_DIR, REPORTS_DIR, THUMBNAIL_DIR, USE_OPENAI_API, MAX_FILE_SIZE, ALLOWED_EXTENSIONS
...
async def process_file_async(session_id: str, file_path: Path, file_type: str):
    ...
    try:
        ...
        try:
            ...
            from models.confidence_scorer import confidence_scorer
...
def get_or_create_user(user_email: Optional[str], user_uid: Optional[str], user_display_name: Optional[str]) -> Optional[int]:
    ...
    try:
        ...
        if not user:
            ...
            logger.info("Created new backend user for Firebase user: %s", user_email)
        return user.id
    except Exception as ex:
        session.rollback()
        logger.warning("Could not associate Firebase user %s: %s", user_email, ex)
        return None
    finally:
        session.close()
...
async def get_processing_status(session_id: str):
    ...
    return JSONResponse(processing_status[session_id])

@app.get("/report/{session_id}", response_class=HTMLResponse)
async def show_report(request: Request, session_id: str):
    """Display the analysis report"""
    try:
        logger.info(f"Report request for session: {session_id}")
        ...
        if session_id in processing_status:
            ...
            if status["status"] == "processing":
                logger.info(f"Session {session_id} still processing")
                ...
                error_msg = status.get("error", "Processing failed")
                logger.error(f"Session {session_id} failed: {error_msg}")
                raise HTTPException(status_code=500, detail=error_msg)
            elif status["status"] == "completed":
                report = status.get("report", {})
                logger.info(f"Session {session_id} completed, report keys: {list(report.keys())}")
        else:
            # Try to load from file
            logger.info(f"Session {session_id} not in memory, loading from file")
            report = report_generator.load_report(session_id)
            if not report:
                logger.error(f"No report found for session: {session_id}")
                raise HTTPException(status_code=404, detail="Report not found")
            logger.info(f"Loaded report from file, keys: {list(report.keys())}")
        ...
        logger.error(f"Error showing report for session {session_id}: {e}", exc_info=True)
...
async def download_report(session_id: str, format: str = "json"):
    """Download report in JSON, PDF, or Word format"""
    try:
        # Load report data
        report = report_generator.load_report(session_id)
        if not report:
        ...
        if format == "pdf":
            # Generate PDF report
            try:
                pdf_path = pdf_word_generator.generate_pdf_report(session_id, report)
                if not pdf_path:
                    logger.error(f"PDF generation returned None for session: {session_id}")
                    raise HTTPException(status_code=500, detail="PDF generation failed - generator returned None")
                
                if not pdf_path.exists():
                    logger.error(f"PDF file does not exist at: {pdf_path}")
                    raise HTTPException(status_code=500, detail="PDF generation failed - file was not created")
                ...
                logger.info(f"Serving PDF file: {pdf_path} (size: {file_size} bytes)")
                
                from fastapi.responses import FileResponse
                return FileResponse(
                    path=str(pdf_path),
                    media_type="application/pdf",
                    filename=f"Face2Phase_Report_{session_id}.pdf",
                    headers={
                        "Content-Disposition": f"attachment; filename=Face2Phase_Report_{session_id}.pdf",
            ...
            word_path = pdf_word_generator.generate_word_report(session_id, report)
            ...
            return FileResponse(
                ...
                filename=f"Face2Phrase_Report_{session_id}.docx"
            )
            ...
            return JSONResponse(
                content=report,
                headers={"Content-Disposition": f"attachment; filename=face2phrase_report_{session_id}.json"}
            )
...
async def get_timestamped_transcript(session_id: str):
    ...
    try:
        ...
        # Try to get audio results from processing status if available
        audio_results = {}
        if session_id in processing_status and 'audio_results' in processing_status[session_id]:
            ...
            # Fallback: reconstruct from report
            audio_results = {
...
@app.get("/api/firebase/analyses")  # Also support GET for compatibility
async def get_firebase_user_analyses(
    request: Optional[FirebaseUserRequest] = None,
    limit: int = 1000,
    email: Optional[str] = None,
    uid: Optional[str] = None,
    display_name: Optional[str] = None
...
async def enhanced_analysis(request: EnhancedAnalysisRequest, user = Depends(get_current_user_from_header)):
    """Get enhanced analysis with OpenAI integration"""
    try:
        # Get analysis data
        analysis_data = db_manager.get_analysis_by_session(request.session_id)
        if not analysis_data:
            raise HTTPException(status_code=404, detail="Analysis not found")
        
        # Verify ownership
        if analysis_data.get('user_id') != user.id:
            raise HTTPException(status_code=403, detail="Access denied")
        
        # Generate enhanced feedback
        enhanced_feedback = openai_enhancer.generate_advanced_feedback(analysis_data)
        ...
        raise HTTPException(status_code=500, detail="Failed to generate enhanced analysis.")
