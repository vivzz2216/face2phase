...
class EnhancedAudioAnalyzer:
    ...
    def __init__(self):
        ...
        if FORCE_GPU_ANALYSIS and self.device != "cuda":
            raise RuntimeError(
                "GPU-only analysis mode is enabled, but Whisper device is not CUDA."
            )
    ...
    def _load_whisper_model(self):
        ...
        if whisper is None:
            logger.error("Whisper library not installed; audio transcription will be disabled.")
            logger.error("Install with: pip install openai-whisper")
            if FORCE_GPU_ANALYSIS:
                raise RuntimeError("GPU-only mode requires `openai-whisper` to be installed.")
            self.whisper_model = None
            return False
        try:
            ...
            logger.info("Whisper is ready for transcription!")
            return True
        except Exception as e:
            ...
            if FORCE_GPU_ANALYSIS:
                raise RuntimeError(f"Failed to load Whisper on CUDA: {e}") from e
            self.whisper_model = None
            return False
