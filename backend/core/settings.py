...
# Load .env when settings are imported (works for app, scripts, and tests)
try:
...
# GPU-only analysis mode (enabled by default, can be disabled via env var)
FORCE_GPU_ANALYSIS = os.getenv("FORCE_GPU_ANALYSIS", "1").strip().lower() in ("1", "true", "yes", "on")
