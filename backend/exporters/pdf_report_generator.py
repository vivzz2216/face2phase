...
class PDFWordReportGenerator:
    ...
    def generate_pdf_report(self, session_id: str, analysis_data: Dict) -> Optional[Path]:
        ...
        try:
            ...
            # Enhanced custom styles with professional design
            ...
            story.append(Paragraph("FACE2PHASE", title_style))
            story.append(Paragraph("Communication Analysis Report", subtitle_style))
            story.append(Spacer(1, 8))
            
            # Session Info with elegant design
            timestamp = analysis_data.get('timestamp', datetime.now().isoformat())
            try:
                timestamp_formatted = datetime.fromisoformat(timestamp.replace('Z', '+00:00')).strftime('%B %d, %Y at %I:%M %p')
            except:
            ...
            story.append(Paragraph(f"Session: {session_id[:16]}... | {timestamp_formatted}", info_style))
            story.append(Spacer(1, 25))
            
            # Overall Score Section with modern gradient-style design
            overall_score = analysis_data.get('overall_score', 0)
            score_color = '#10b981' if overall_score >= 70 else '#f59e0b' if overall_score >= 50 else '#ef4444'
            ...
            score_box_style = TableStyle([
                ...
                ('TEXTCOLOR', (0, 1), (-1, 1), colors.HexColor(score_color)),
            ...
            score_box = Table([['Overall Communication Score'], [f"{overall_score:.0f}/100"]], 
                            colWidths=[5.5*inch], rowHeights=[0.45*inch, 0.9*inch])
            score_box.setStyle(score_box_style)
            story.append(score_box)
            story.append(Spacer(1, 25))
            
            # Scores Table with modern styling (conditionally hide facial confidence for audio-only)
            ...
            scores_data = [
                ['METRIC', 'SCORE', 'RATING'],
                ['Voice Confidence', f"{voice_score:.0f}/100", self._get_status(voice_score)],
            ]
            
            # Only add facial confidence if NOT audio-only
            if not analysis_data.get('is_audio_only', False):
                scores_data.append(['Facial Confidence', f"{facial_score:.0f}/100", self._get_status(facial_score)])
            
            scores_data.append(['Vocabulary Score', f"{vocab_score:.0f}/100", self._get_status(vocab_score)])
            
            scores_table = Table(scores_data, colWidths=[2.8*inch, 1.4*inch, 1.3*inch])
            ...
            if 'strict_evaluation' in analysis_data:
                ...
                # Red Flags Applied - handle both dict and list types
                flags = strict_eval.get('flags', {})
                # Ensure flags is a dict before calling .values()
