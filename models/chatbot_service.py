...
class ChatbotService:
    ...
    def _get_ai_response_stream(self, messages: List[Dict]) -> Generator[str, None, None]:
        """Yield streaming chunks from OpenAI API."""
        try:
    ...
    def _prepare_system_context(self, report_data: Dict) -> str:
        """Prepare system context for chatbot"""
        context_parts = [
            "You are a helpful AI assistant providing guidance about a presentation video analysis.",
            "You can answer questions about the presentation, analysis scores, feedback, and provide suggestions.",
            "",
            "ANALYSIS SUMMARY:"
        ]
        audio_metrics = report_data.get('audio_analytics', {})
        visual_metrics = report_data.get('visual_analytics', {})
        ...
        if audio_metrics:
            opening = audio_metrics.get('opening_confidence', {})
            if opening:
                context_parts.append(f"- Opening Confidence: {opening.get('opening_confidence', 0)}/100 with "
                                     f"{opening.get('opening_filler_count', 0)} fillers in first 30s")
        if visual_metrics:
            tension = visual_metrics.get('tension_summary', {})
            if tension:
                context_parts.append(f"- Tension Ratio: {tension.get('tension_percentage', 0)}% tension, "
                                     f"eye-contact stability {tension.get('eye_contact_stability', 'N/A')}")
        if text_metrics:
            context_parts.append(f"- Topic Coherence: {text_metrics.get('topic_coherence_score', 0)} / 100")
            coverage = text_metrics.get('keyword_coverage', {})
            if coverage:
                context_parts.append(f"- Keyword Coverage: {coverage.get('total_keywords', 0)} keywords "
                                     f"({coverage.get('keyword_density', 0)}% density)")
        
        # Add transcript preview
        transcript = report_data.get('transcript', '')
        if transcript:
            transcript_preview = transcript[:1000]  # First 1000 chars
        ...
        context_parts.append("\nWhen answering, adopt the tone of an experienced presentation coach at a global consultancy.")
        context_parts.append("Reference specific timestamps and metrics when possible. Suggest drills, frameworks, or follow-up prompts.")
