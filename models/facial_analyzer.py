...
from config import FRAME_EXTRACTION_FPS, MIN_FACE_CONFIDENCE
    ...
    def analyze_eye_contact(self, frame: np.ndarray, face_box: Tuple[int, int, int, int]) -> Dict:
        ...
        try:
            if not self.landmark_predictor:
                return {'eye_contact_score': 0.5, 'gaze_direction': 'unknown'}
            ...
            # Simple gaze estimation (can be improved)
            gaze_direction = 'center'
            if avg_ear < 0.2:  # Eyes might be closed or looking away
                gaze_direction = 'away'
            elif avg_ear > 0.3:  # Eyes open, likely looking at camera
                gaze_direction = 'direct'
            
            return {
                'eye_contact_score': min(1.0, avg_ear * 3),  # Normalize to 0-1
