...
class ProfessionalPDFExporter:
    ...
    def build_template_context(self, session_id: str, report: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        filler_count = report.get("filler_word_count", 0)
        filler_ratio = (report.get("filler_word_ratio") or 0) * 100
        speaking_rate = report.get("speaking_rate_wpm") or safe_get(report, "speaking_metrics", "speaking_rate_wpm", default=0)
        weak_words = safe_get(report, "word_analysis", "weak_words", "weak_word_count", default=0)
            ...
            for line in transcript_raw.splitlines():
                ...
                if ":" in stripped[:5]:
                    ...
                    if len(parts) == 2 and parts[0].count(":") == 1:
                        transcript_lines.append({"time": parts[0], "text": parts[1]})
                        continue
        ...
        if not follow_up_questions:
            follow_up_questions = [
                "What is one story that could make this introduction more memorable?",
                "How can you summarise your background in one impactful sentence?",
                "What visual cue will you use to remind yourself to smile naturally?"
            ]
        ...
        return {
            ...
            "visual": {
                ...
                "emotion_timeline": safe_get(report, "visual_analytics", "emotion_timeline_smoothed", default=[])
            },
            "text": {
