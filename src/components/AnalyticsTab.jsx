...
const AdvancedAnalytics = ({
  ...
  const pauseCadence = audioAnalytics.pause_cadence || {}
  ...
  const renderTimelineSnippet = () => {
    ...
    const snippet = emotionTimelineSmoothed.slice(0, 5)
    return (
      <ul className="trend-list">
        ...
        {emotionTimelineSmoothed.length > 5 && (
          <li className="muted">+{emotionTimelineSmoothed.length - 5} more entries</li>
        )}
      </ul>
    )
  }
      ...
      <div className="advanced-section">
        ...
        <div className="advanced-grid">
          ...
          <div className="advanced-card">
              ...
              {['short', 'medium', 'long'].map((bucket) => (
                <div key={bucket} className="pause-cell">
                  ...
                  <div className="pause-duration">
                    {pauseDurations[bucket] ? `${pauseDurations[bucket].toFixed(1)}s` : '--'}
                  </div>
                </div>
              ))}
            </div>
            <p className="muted">
              Avg pause length:{' '}
              {typeof pauseCadence.average_duration === 'number'
                ? `${pauseCadence.average_duration.toFixed(2)}s`
                : '--'}
            </p>
          </div>
